...
module RedmineHelpdeskGPG
	module Patches
		...
		module HelpdeskMailContainerPatch
			...
			CONTAINER_ACCESSORS = [:issue, :is_new_issue].freeze
			...
			module InstanceMethods
				...
				def initialize_with_gpg(email_or_raw, options = {})
					...
					if the_email.encrypted?
						...
						#_myLogger.info "receive_with_gpg: Mail was not signed nor encrypted" unless _myLogger.nil?
					end
					return initialize_without_gpg(the_email, options) # call original method
				...
				def saveGpgJournal(ref, options)
					if @gpg_received_options[:signed] || @gpg_received_options[:encrypted] then
